...
private parseBilingualSummary(content: string, viewIndex: 1 | 2): { english: string; german: string } | null {
  const queryApi = viewIndex === 1 ? this.queryApi1 : this.queryApi2;
  if (!content || queryApi !== 'german') return null;
    ...
    this.allBins[newQueryIndex] = [
      {
        label: "50 - 100",
        count: response.results.filter((d: any) => d.similarity_score >= 50 && d.similarity_score <= 100).length,
        data: response.results.filter((d: any) => d.similarity_score >= 50 && d.similarity_score <= 100).map(tagV1),
        color: 'green'
      },
      {
        label: "25 - 50",
        count: response.results.filter((d: any) => d.similarity_score >= 25 && d.similarity_score < 50).length,
        data: response.results.filter((d: any) => d.similarity_score >= 25 && d.similarity_score < 50).map(tagV1),
        color: 'orange'
      },
      {
        label: "0 - 25",
        count: response.results.filter((d: any) => d.similarity_score >= 0 && d.similarity_score < 25).length,
        data: response.results.filter((d: any) => d.similarity_score >= 0 && d.similarity_score < 25).map(tagV1),
        color: 'red'
      }
    ];
  } else {
    const view1Bin0 = response.results.filter((d: any) => d.similarity_score >= 50 && d.similarity_score <= 100).map(tagV1);
    const view1Bin1 = response.results.filter((d: any) => d.similarity_score >= 25 && d.similarity_score < 50).map(tagV1);
    const view1Bin2 = response.results.filter((d: any) => d.similarity_score >= 0 && d.similarity_score < 25).map(tagV1);
...
if (this.allBins[newQueryIndex]) {
  ...
  const view2Bin0 = response2.results.filter((d: any) => d.similarity_score >= 50 && d.similarity_score <= 100).map(tagV2);
  const view2Bin1 = response2.results.filter((d: any) => d.similarity_score >= 25 && d.similarity_score < 50).map(tagV2);
  const view2Bin2 = response2.results.filter((d: any) => d.similarity_score >= 0 && d.similarity_score < 25).map(tagV2);
  ...
  this.allBins[newQueryIndex] = [
    {
      label: "50 - 100",
      count: response2.results.filter((d: any) => d.similarity_score >= 50 && d.similarity_score <= 100).length,
      data: response2.results.filter((d: any) => d.similarity_score >= 50 && d.similarity_score <= 100).map(tagV2Init),
      color: 'green'
    },
    {
      label: "25 - 50",
      count: response2.results.filter((d: any) => d.similarity_score >= 25 && d.similarity_score < 50).length,
      data: response2.results.filter((d: any) => d.similarity_score >= 25 && d.similarity_score < 50).map(tagV2Init),
      color: 'orange'
    },
    {
      label: "0 - 25",
      count: response2.results.filter((d: any) => d.similarity_score >= 0 && d.similarity_score < 25).length,
      data: response2.results.filter((d: any) => d.similarity_score >= 0 && d.similarity_score < 25).map(tagV2Init),
      color: 'red'
    }
  ];
}
...
if (isNewQuery) {
  ...
  private getPathFromTopicPath(topicPath: string): string {
    return String(topicPath || '').split('/').filter(Boolean).join(' --> ');
  }
