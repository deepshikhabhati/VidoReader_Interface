...
import { Full } from 'src/assets/Full';
...
@Component({
  ...
  templateUrl: './main-content-full.component.html',
  styleUrls: ['./main-content-full.component.css']
})
export class MainContentFullComponent implements OnInit, AfterViewChecked, AfterViewInit {
  /** Principles of Management (Full.ts) hierarchy for topic list & charts */
  topicData = Full.children
  chartData = Full
  intersection: any = 0
  nodeName: any = ''
  barChartValue: any = []
  ...
  circleTreeSize = { width: 700, height: 400 };
  ...
  setTopic(topic: any): void {
    ...
    const existingIndex = this.selectedTopicContents.findIndex(t => t.name === topic.name);
  ...
  /** Default queries for the Full (management) corpus; align with how Full-queryResultts was scored (score1/score2/score3). */
  predefinedQueries: string[] = [
    "What is the P-O-L-C framework and how does it structure a manager's work?",
    "What is the difference between intended, emergent, and realized strategy?",
    "What characteristics make goals effective, and what does SMART criteria entail?",
    "What roles do mission and vision play within the P-O-L-C planning framework?",
    "How does SWOT analysis combine internal and external analysis to shape strategy?",
    "What is a realized strategy and how does it form inside an organization?",
    "Why do organizations end up with a different strategy than the one they originally planned?",
    "How should managers stay flexible during strategy implementation to account for unexpected changes?",
  ];

  /** Static illustration for Module Keywords when query + corpus slice match. */
  realizedStrategyKeywordsImageUrl = 'assets/wordCloud.png';
  private readonly realizedStrategyQueryText =
    'What is a realized strategy and how does it form inside an organization?';
  private readonly strategyInManagementTopicPath = 'Strategizing/Strategy in Management';

  private normalizeQueryText(s: string): string {
    return (s || '').trim().replace(/\s+/g, ' ');
  }

  /** Topic path from assistant message (persisted or derived from sunburst path). */
  getMessageTopicPath(message: any): string {
    if (message?.topic_path) {
      return String(message.topic_path).trim();
    }
    if (!message?.path) {
      return '';
    }
  ...
  /** Show fixed keyword image instead of canvas word cloud for this query + module. */
  shouldShowStrategyKeywordImage(message: any): boolean {
    const q = this.normalizeQueryText(
      message?.userQueryText ?? this.getCurrentQueryMessage()
    );
    if (q !== this.normalizeQueryText(this.realizedStrategyQueryText)) {
      return false;
    }
    return this.getMessageTopicPath(message) === this.strategyInManagementTopicPath;
  }
  ...
  sendMessage(msg: any) {
    if (this.userMessage.trim() || this.selectedQuery) {
      ...
      if(!this.selectedUSerMessage2 && !this.selectedUSerMessage3) {
        ...
        this.openaiService.sendQueryFullReduced(msg,this.numberOfChunks).subscribe((response: any) => {
            this.querysent = true
...
if (isNewQuery) {
  ...
  renderMessageWordCloud(message: any, messageIndex: number): void {
    ...
    if (this.shouldShowStrategyKeywordImage(message)) {
      return;
    }
    ...
    this.openaiService.askQuery(
      ...
      if (res.response) {
        ...
        this.chatMessages.push({
          ...
          topic_path: firstMatchedNode.topic_path,
          userQueryText: queryMessageForAI,
          no: queryNum,
