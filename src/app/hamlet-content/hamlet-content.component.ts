import { Component, ElementRef, ViewChild, HostListener, AfterViewChecked, Input, OnInit } from '@angular/core';
...
@Component({
  ...
  templateUrl: './hamlet-content.component.html',
  styleUrls: ['./hamlet-content.component.css']
})
export class HamletContentComponent implements OnInit, AfterViewChecked {
  private _chartData: any = HAMLET;
  topicData: any[] = HAMLET.children;

  @Input() set chartData(value: any) {
    this._chartData = value || HAMLET;
    this.topicData = this._chartData?.children || [];
  }
  ...
  @Input() queryResultsData: any[] = defaultQueryResults;
  @Input() queryApi: 'default' | 'german' = 'default';
  @Input() apiQueries: string[] = [];
  intersection: any = 0
  nodeName: any = ''
  nodePath: any = ''
  barChartValue: any = []
  ...
  // Floating chart windows properties
  showCircleTreeWindow: boolean = false;
  showSunburstWindow: boolean = false;
  circleTreeExpanded: boolean = true;
  sunburstExpanded: boolean = true;
  circleTreePosition = { x: 100, y: 100 };
  sunburstPosition = { x: 400, y: 100 };
  circleTreeSize = { width: 400, height: 300 };
  sunburstSize = { width: 400, height: 300 };
  isDraggingWindow: boolean = false;
  draggingWindow: 'circle-tree' | 'sunburst' | null = null;
  windowDragOffset = { x: 0, y: 0 };
  isResizingWindow: boolean = false;
  resizingWindow: 'circle-tree' | 'sunburst' | null = null;
  ...
  constructor(private openaiService: OpenaiService, private sanitizer: DomSanitizer) {
   ...
   console.log("🔥 Initial selectedBinIndex:", this.selectedBinIndex);
    ...
    // Hamlet-specific initialization
...
private sendSimilarityQuery(msg: any) {
  if (this.queryApi === 'german') {
    return this.openaiService.sendQueryGerman(msg, this.numberOfChunks);
  }

  return this.openaiService.sendQuery4(msg, this.numberOfChunks);
}

private getApiQueryForIndex(queryIndex: number): string {
  return this.apiQueries[queryIndex] || this.predefinedQueries[queryIndex] || '';
}

private getApiQueryForDisplayQuery(query: string): string {
  const queryIndex = this.predefinedQueries.indexOf(query);
  return queryIndex >= 0 ? this.getApiQueryForIndex(queryIndex) : query;
}

private getAskQueryOptions(): { german?: boolean; language?: string } | undefined {
  return this.queryApi === 'german' ? { german: true, language: 'german' } : undefined;
}

private parseBilingualSummary(content: string): { english: string; german: string } | null {
  if (!content || this.queryApi !== 'german') return null;

  const englishMatch = content.match(/English:\s*([\s\S]*?)(?=\n\s*German:|$)/i);
  const germanMatch = content.match(/German:\s*([\s\S]*)/i);
  const english = englishMatch?.[1]?.trim() || '';
  const german = germanMatch?.[1]?.trim() || '';

  return english || german ? { english, german } : null;
}
  ...
  sendMessage(msg: any) {
    if (this.userMessage.trim() || this.selectedQuery) {
      ...
      const apiMessage = this.getApiQueryForDisplayQuery(msg.trim());
      if(this.selectedUSerMessage && this.selectedUSerMessage2) {
        this.selectedUSerMessage3 = apiMessage;
      } else if(this.selectedUSerMessage) {
        this.selectedUSerMessage2 = apiMessage;
        // this.fuzzyPopup = true
      } else {
        this.selectedUSerMessage = apiMessage
      }
      ...
      if(!this.selectedUSerMessage2 && !this.selectedUSerMessage3) {
        ...
        this.sendSimilarityQuery(apiMessage).subscribe((response: any) => {
            this.querysent = true
...
this.bins[0] = {
  label: "50 - 100",
  count: response.results.filter((d: any) => d.similarity_score >= 45 && d.similarity_score <= 100).length,
  data: response.results.filter((d: any) => d.similarity_score >= 45 && d.similarity_score <= 100),
  color: 'green'
};

this.bins[1] = {
  label: "25 - 50",
  count: response.results.filter((d: any) => d.similarity_score >= 25 && d.similarity_score < 45).length,
  data: response.results.filter((d: any) => d.similarity_score >= 25 && d.similarity_score < 45),
  color: 'orange'
};
...
this.bins3[2] = {
  ...
  selectedData(data: any) {
    ...
    this.openaiService.askQuery(queryMessage, data.content, this.getAskQueryOptions()).subscribe((res: any) => {
      console.log('AI Response:', res);
      const aiResponse = res.response;
      const summaryByLanguage = this.parseBilingualSummary(aiResponse);
      ...
      this.chatMessages.push({ 
        ...
        content: aiResponse,
        summaryByLanguage,
        summaryLanguage: summaryByLanguage?.english ? 'english' : 'german',
  ...
  public updateSimilarityScores2(treeData: any[], similarityData: any[],query2: any) {
    ...
    sendMessageForQuery(queryIndex: number, binIndex: number): void {
      ...
      } else if (this.queryResultsData && this.queryResultsData.length > 0) {
        // For configured/static queries, use queryResultsData
        if (binIndex === 0) {
          // Bin 0: 50-100
          filteredData = this.queryResultsData.filter((item: any) => {
            const score = item[scoreKey];
            return score !== undefined && score !== null && score >= 45 && score <= 100;
          });
        } else if (binIndex === 1) {
          // Bin 1: 25-50
          filteredData = this.queryResultsData.filter((item: any) => {
            const score = item[scoreKey];
            return score !== undefined && score !== null && score >= 25 && score < 45;
          });
        } else if (binIndex === 2) {
          // Bin 2: 0-25
          filteredData = this.queryResultsData.filter((item: any) => {
      ...
      if (transformedData.length === 0) {
        ...
        if (this.queryResultsData && this.queryResultsData.length > 0) {
          console.log(`Sample queryResults item:`, this.queryResultsData[0]);
      ...
      if (structureQueryNumber === '1') {
        ...
        this.selectedUSerMessage = this.getApiQueryForIndex(queryIndex);
      } else if (structureQueryNumber === '2') {
        this.bins2[binIndex] = {
          label: binIndex === 0 ? '50 - 100' : (binIndex === 1 ? '25 - 50' : '0 - 25'),
          count: filteredData.length,
        ...
        this.selectedUSerMessage2 = this.getApiQueryForIndex(queryIndex);
      } else if (structureQueryNumber === '3') {
        this.bins3[binIndex] = {
          label: binIndex === 0 ? '50 - 100' : (binIndex === 1 ? '25 - 50' : '0 - 25'),
          count: filteredData.length,
        ...
        this.selectedUSerMessage3 = this.getApiQueryForIndex(queryIndex);
      }
    ...
    getBinCountForQuery(queryIndex: number, binIndex: number): number {
      ...
      // Fall back to configured/static query results.
      if (!this.queryResultsData || this.queryResultsData.length === 0) {
        return 0;
      }
      ...
      this.queryResultsData.forEach((item: any) => {
        const score = item[scoreKey];
        if (score !== undefined && score !== null) {
          if (binIndex === 0 && score >= 45 && score <= 100) {
            count++;
          } else if (binIndex === 1 && score >= 25 && score < 45) {
            count++;
...
public updateSimilarityScores(treeData: any[], similarityData: any[], queryNumber: string) {
  ...
  if (queryMessageForAI) {
    this.openaiService.askQuery(
      ...
      this.getAskQueryOptions()
    ).subscribe((res: any) => {
      if (res.response) {
        const summaryByLanguage = this.parseBilingualSummary(res.response);
...
colorPalette = [
 ...
 selectedSegment: number | null = null;
  ...
  // Floating chart window methods
  toggleFloatingChart(chartType: 'circle-tree' | 'sunburst'): void {
    if (chartType === 'circle-tree') {
      ...
      this.showSunburstWindow = !this.showSunburstWindow;
      if (this.showSunburstWindow && !this.sunburstExpanded) {
        this.sunburstExpanded = true;
      }
    }
  }

  closeFloatingChart(chartType: 'circle-tree' | 'sunburst'): void {
    if (chartType === 'circle-tree') {
      this.showCircleTreeWindow = false;
    } else if (chartType === 'sunburst') {
      this.showSunburstWindow = false;
    }
  }

  toggleWindowExpand(chartType: 'circle-tree' | 'sunburst'): void {
    if (chartType === 'circle-tree') {
      this.circleTreeExpanded = !this.circleTreeExpanded;
      // Recalculate dimensions when expanded
      ...
      this.sunburstExpanded = !this.sunburstExpanded;
      // Recalculate dimensions when expanded
      if (this.sunburstExpanded) {
        setTimeout(() => {
          this.recalculateChartDimensions('sunburst');
        }, 200);
      }
    }
  }

  onWindowMouseDown(event: MouseEvent, windowType: 'circle-tree' | 'sunburst'): void {
    ...
    } else if (windowType === 'sunburst') {
      currentPosition = this.sunburstPosition;
    }
  ...
  onWindowMouseMove = (event: MouseEvent): void => {
    if (this.isResizingWindow && this.resizingWindow) {
      ...
      } else if (this.resizingWindow === 'sunburst') {
        if (this.sunburstSize.width !== newWidth || this.sunburstSize.height !== newHeight) {
          this.sunburstSize = { width: newWidth, height: newHeight };
          sizeChanged = true;
        }
      }
      ...
      } else if (this.draggingWindow === 'sunburst') {
        windowWidth = this.sunburstSize.width;
        windowHeight = this.sunburstSize.height;
      }
      ...
      if (this.draggingWindow === 'circle-tree') {
        ...
        this.sunburstPosition = { x: newX, y: newY };
      }
    }
  }
  ...
  private recalculateChartDimensions(windowType: 'circle-tree' | 'sunburst'): void {
    // Use setTimeout to ensure DOM has updated
    setTimeout(() => {
      if (windowType === 'circle-tree' && this.circleTreeChart) {
        this.circleTreeChart.recalculateDimensions();
      } else if (windowType === 'sunburst' && this.sunburstChart) {
        this.sunburstChart.recalculateDimensions();
      }
    }, 100);
  }

  onWindowResizeMouseDown(event: MouseEvent, windowType: 'circle-tree' | 'sunburst'): void {
    event.stopPropagation();
    event.preventDefault();
